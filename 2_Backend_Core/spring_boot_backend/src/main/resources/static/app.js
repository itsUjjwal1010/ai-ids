...
const DEVICE_CONFIG = {
  'esp32-gw':  { name: 'ESP32 Gateway', ip: '192.168.121.6',   icon: '📡', role: 'Traffic Sniffer / AP Host' },
  'esp32-cam': { name: 'ESP32-CAM',     ip: '192.168.121.167', icon: '📹', role: 'IP Camera / IoT Node', streamUrl: 'http://192.168.121.167/stream' },
  'dht11':     { name: 'DHT11 Sensor',  ip: '192.168.121.6',   icon: '🌡️', role: 'Temperature & Humidity (on Gateway)' },
};
...
function connectWS() {
  try {
    ...
    stompClient.connect({}, () => {
      setWsStatus(true);
      stompClient.subscribe('/topic/packets', msg => onPacket(JSON.parse(msg.body)));
      stompClient.subscribe('/topic/devices', msg => onDeviceUpdate(JSON.parse(msg.body)));
      stompClient.subscribe('/topic/sensor',  msg => { onSensorData(msg); });
      fetch('/api/devices').then(r => r.json()).then(onDeviceUpdate).catch(() => {});
      // Do NOT auto-start simulation — dashboard only shows real ESP32 data
    }, () => { setWsStatus(false); setTimeout(connectWS, 3000); });
  } catch(e) { setWsStatus(false); setTimeout(connectWS, 5000); }
}
...
function updateDetectionsTable(pkt) {
  ...
  tr.innerHTML =
    ...
    '<td class="mono">' + (pkt.port>0?pkt.port:'&mdash;') + '</td>' +
...
function renderLogPage() {
  ...
  filtered.slice(0, 300).forEach(function(pkt) {
    ...
    var layer = pkt.attackLayer || (pkt.attack ? 'L4' : 'NONE');
    var lBg = layer==='L7'?'#7c3aed':layer==='L4'?'#dc2626':layer==='L3'?'#d97706':'#6b7280';
...
function exportLogs() {
  const h = 'Timestamp,SourceIP,DestIP,Protocol,Port,Size,Type,Label,Confidence,Severity\n';
  const rows = STATE.allPackets.map(p =>
    `${p.timestamp},${p.sourceIp},${p.destIp},${p.protocol},${p.port},${p.packetSize},${p.attackType},${CIC_LABELS[p.attackType]||''},${Math.round((p.confidence||0)*100)}%,${p.severityScore}`
  ).join('\n');
...
async function toggleSim() {
  ...
  try {
    const res = await fetch(running ? '/api/simulation/stop' : '/api/simulation/start', { method: 'POST' });
...
async function launchSimulationAttack() {
  ...
  for (let i = 0; i < count; i++) {
    await fetch('/api/inject?type=' + type, { method: 'POST' });
    STATE.simInjected++;
...
// ── ROUTER ────────────────────────────────────────────────────────────────────
const PAGE_TITLES = {
  dashboard: 'Dashboard', logs: 'Attack Logs', devices: 'IoT Devices',
...
const PAGE_SUBS = {
  ...
  devices:   'Live hardware status · 192.168.4.0/24 · Zero PCB Assembly',
...
window.launchCustomAttack = function(type) {
  let url = '/api/attack/start?type=' + type;
...
const MITRE_TECHNIQUES = [
  { tactic: 0, id:'T1595', name:'Active Scanning',       attacks:['RECON','PORT_SCAN','Recon'] },
  { tactic: 0, id:'T1590', name:'Gather Network Info',   attacks:['RECON','Recon'] },
  { tactic: 1, id:'T1190', name:'Exploit Public App',    attacks:['MITM','ARP_SPOOF','Spoofing'] },
  { tactic: 1, id:'T1133', name:'External Services',     attacks:['MITM','ARP_SPOOF'] },
  { tactic: 2, id:'T1059', name:'Command Interpreter',   attacks:['MIRAI','Mirai'] },
  { tactic: 2, id:'T1203', name:'Exploitation',          attacks:['DOS','DoS','DDoS','UDP_FLOOD'] },
  { tactic: 3, id:'T1505', name:'Server Compromise',     attacks:['MIRAI','Mirai'] },
  { tactic: 3, id:'T1547', name:'Boot Persistence',      attacks:['MIRAI','Mirai'] },
  { tactic: 4, id:'T1498', name:'Network DoS',           attacks:['DOS','DoS','DDoS','UDP_FLOOD','DDOS'] },
  { tactic: 4, id:'T1499', name:'Endpoint DoS',          attacks:['DOS','DoS','DDoS','UDP_FLOOD'] },
  { tactic: 5, id:'T1095', name:'Non-App Layer C2',      attacks:['MIRAI','Mirai','ARP_SPOOF'] },
  { tactic: 5, id:'T1571', name:'Non-Standard Port',     attacks:['MITM','ARP_SPOOF','Spoofing'] },
  { tactic: 6, id:'T1048', name:'Exfil Over Channel',    attacks:['MITM','Spoofing','ARP_SPOOF'] },
  { tactic: 6, id:'T1041', name:'Exfil Over C2',         attacks:['MIRAI','Mirai'] },
];
